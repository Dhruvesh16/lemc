...
class GRUTrajectoryModel(nn.Module):
    ...
    Consumes a single pedestrian's [B, T_obs, 4] track only -- no other agents,
    no ego-motion compensation. Any improvement from LEMC must be attributable
    to LEMC alone, not to this backbone."""
    ...
    def forward(self, track: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor | None]:
        """track: [B, T_obs, 4] -> traj_pred [B, T_pred, 2] (cx, cy only), intent_logit [B, 1] or None."""
        x = self.embed(track)
        _, h_n = self.gru(x)
